...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const MAPTILER_API_KEY = 'F3cdRiC1r36tcrNrvrcV';
    // REMEMBER TO REPLACE THIS WITH YOUR OWN OPENWEATHERMAP API KEY
    ...
    map.on('load', () => {
        ...
        map.addSource('traffic', {
            type: 'raster',
            tiles: [
                `https://api.maptiler.com/tiles/traffic/{z}/{x}/{y}.png?key=${MAPTILER_API_KEY}`
            ],
            tileSize: 256
        });
        map.addLayer({
    ...
    // FIX 1: Ensure search listener is attached to the main search input
    ...
    getRouteBtn.addEventListener('click', async () => {
        ...
        const url = `https://api.maptiler.com/directions/v5/mapbox/driving/${fromCoord.join(',')};${toCoord.join(',')}?geometries=geojson&overview=full&steps=true&access_token=${MAPTILER_API_KEY}`;
        try {
            const response = await fetch(url);
            const data = await response.json();
    ...
    // --- WIKIPEDIA & WEATHER LOGIC (COMPLETED) ---
    ...
    async function showInfoPanel(place) {
        ...
        const headerImageEl = document.querySelector('.header-image');
        factsEl.innerHTML = 'Loading...';
        weatherEl.innerHTML = '<span class="loading">Loading...</span>';
        headerImageEl.src = '';
        headerImageEl.alt = '';
        headerImageEl.style.display = 'none';
        // Fetch Weather
        const weatherData = await getWeather(place.lat, place.lon);
        if (weatherData && weatherData.main && weatherData.weather) {
            weatherEl.innerHTML = `
                <img src="https://openweathermap.org/img/wn/${weatherData.weather[0].icon}@2x.png" alt="${weatherData.weather[0].description}" class="weather-icon">
                <div class="weather-info">
                    <span class="temperature">${Math.round(weatherData.main.temp)}°F</span>
                    <span class="description">${weatherData.weather[0].main}</span>
                </div>
            `;
        } else {
            weatherEl.innerHTML = '<span class="error">Weather unavailable.</span>';
        }
        // Fetch Wikipedia Summary
        if (place.extratags && place.extratags.wikipedia) {
            const wikipediaTitle = place.extratags.wikipedia.split(':').pop();
            const wikiData = await getWikipediaSummary(wikipediaTitle);
            if (wikiData && wikiData.extract) {
                factsEl.textContent = wikiData.extract;
            } else {
                factsEl.textContent = 'No quick facts available.';
            }
            if (wikiData && wikiData.originalimage && wikiData.originalimage.source) {
                headerImageEl.src = wikiData.originalimage.source;
                headerImageEl.style.display = 'block';
            }
        } else {
            factsEl.textContent = 'No Wikipedia page found.';
        }
    }
});

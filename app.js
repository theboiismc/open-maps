document.addEventListener('DOMContentLoaded', () => {
    ...
    const mobileClosePanelBtn = document.getElementById('mobile-close-panel-btn'); // NEW ELEMENT
    ...
    function showPanel(viewId) {
        ...
        if (isMobile) {
            ...
            topSearchWrapper.classList.add('hidden-by-panel'); // HIDE SEARCH BAR
        } else {
    ...
    function closePanel() {
        if (isMobile) {
            ...
            topSearchWrapper.classList.remove('hidden-by-panel'); // SHOW SEARCH BAR
        } else {
    ...
    mobileClosePanelBtn.addEventListener('click', closePanel); // NEW LISTENER
    map.on('click', (e) => {
        const targetClasses = e.originalEvent.target.classList;
        if (!targetClasses.contains('maplibregl-ctrl-icon') && !targetClasses.contains('mapboxgl-ctrl-icon')) { closePanel(); }
    });
    ...
    function attachSuggestionListener(inputEl, suggestionsEl, onSelect) {
        const fetchAndDisplaySuggestions = async (query) => {
            ...
            try {
                ...
                data.forEach(item => { const el = document.createElement("div"); el.className = "search-result"; el.textContent = item.display_name; el.addEventListener("click", () => onSelect(item)); suggestionsEl.appendChild(el); });
    ...
    async function fetchAndSetPlaceImage(query, lon, lat) {
        ...
        imgEl.src = ''; imgEl.style.backgroundColor = '#e0e0e0'; imgEl.alt = 'Loading image...'; imgEl.onerror = null;
        try {
    ...
    function getWeatherDescription(code) { const d = { 0: 'Clear sky', 1: 'Mainly clear', 2: 'Partly cloudy', 3: 'Overcast', 45: 'Fog', 48: 'Depositing rime fog', 51: 'Light drizzle', 53: 'Moderate drizzle', 55: 'Dense drizzle', 61: 'Slight rain', 63: 'Moderate rain', 65: 'Heavy rain', 71: 'Slight snow fall', 73: 'Moderate snow fall', 75: 'Heavy snow fall', 80: 'Slight rain showers', 81: 'Moderate rain showers', 82: 'Violent rain showers', 95: 'Thunderstorm', 96: 'Thunderstorm with slight hail', 99: 'Thunderstorm with heavy hail' }; return d[code] || "Weather data unavailable"; }
    async function fetchAndSetWeather(lat, lon) {
        const weatherEl = document.getElementById('info-weather'); weatherEl.textContent = "Loading weather...";
        try {
            const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}¤t_weather=true&temperature_unit=fahrenheit`;
            const res = await fetch(url); if (!res.ok) throw new Error(`API returned status ${res.status}`);
            const data = await res.json();
            if (data.current_weather) { const tempF = Math.round(data.current_weather.temperature); const tempC = Math.round((tempF - 32) * 5 / 9); const description = getWeatherDescription(data.current_weather.weathercode); weatherEl.textContent = `${tempF}°F / ${tempC}°C, ${description}`; }
    ...
    async function fetchAndSetQuickFacts(query) {
        const factsEl = document.getElementById('quick-facts-content'); factsEl.textContent = "Loading facts...";
        try {
    ...
    function openDirectionsPanel() {
        ...
        if (currentPlace) { toInput.value = currentPlace.display_name; toInput.dataset.coords = `${currentPlace.lon},${currentPlace.lat}`; fromInput.value = ''; fromInput.dataset.coords = ''; }
        else { toInput.value = mainSearchInput.value; toInput.dataset.coords = ''; fromInput.value = ''; fromInput.dataset.coords = ''; }
    }
    ...
    document.getElementById('get-route-btn').addEventListener('click', async () => {
        ...
        try {
            ...
            const res = await fetch(url); const data = await res.json(); if (!data.routes || data.routes.length === 0) return alert("No route found.");
            const route = data.routes[0].geometry; const routeGeoJSON = { type: 'Feature', geometry: route }; currentRouteGeoJSON = routeGeoJSON; addRouteToMap(routeGeoJSON);
            const bounds = new maplibregl.LngLatBounds(); route.coordinates.forEach(coord => bounds.extend(coord)); map.fitBounds(bounds, { padding: isMobile ? { top: 50, bottom: 250, left: 50, right: 50 } : 100 });
            const stepsEl = document.getElementById("route-steps"); stepsEl.innerHTML = "";
            data.routes[0].legs[0].steps.forEach(step => { const li = document.createElement("li"); li.textContent = step.maneuver.instruction; stepsEl.appendChild(li); });
    ...
    if (isMobile) {
        ...
        document.addEventListener('touchmove', (e) => {
            ...
            if (sidePanel.classList.contains('open') && diff > 0) {
                sidePanel.style.bottom = `${-diff}px`;
            }
        }, { passive: true });
        document.addEventListener('touchend', (e) => {
            ...
            if (sidePanel.classList.contains('open') && diff > 80) { // If dragged down more than 80px
                closePanel();
            }
        });
    }

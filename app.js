...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('info-save-btn').addEventListener('click',()=>{
        if(currentUser) showToast("Feature 'Save Place' not implemented","info");
        else showToast("Please log in to save places","warning");
    });
    ...
    async function getRoute(){
        if(!fromInput.value || !toInput.value) return showToast("Please fill both start and end points","warning");
        clearRouteFromMap();
        try{
            ...
            if(!data.routes||data.routes.length===0||!data.routes[0].legs||!data.routes[0].legs[0].steps||data.routes[0].legs[0].steps.length===0){
                return showToast("A route could not be found.","warning");
            }
        ...
        } catch(err){ showToast(`Error getting route: ${err.message}`,"error"); navigationState.isRerouting=false; }
    }
    ...
    shareRouteBtn.addEventListener('click',async()=>{
        ...
        if(navigator.share){
            ...
            catch(error){ console.error('Error sharing:',error); showToast("Could not share route","error"); }
        } else {
            navigator.clipboard.writeText(url.toString()).then(()=>{ showToast("Route link copied!","info"); })
            .catch(err=>{ console.error('Copy failed: ',err); showToast("Could not copy route link","error"); });
        }
    });

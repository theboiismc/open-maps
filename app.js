...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function updateNavigationUI() {
    const remainingTime = Math.max(0, (navigationState.totalTripTime / 60));
    statTimeRemainingEl.textContent = `${remainingTime.toFixed(0)} min`;
    statEtaEl.textContent = formatEta(navigationState.estimatedArrivalTime);
    statSpeedEl.textContent = (navigationState.userSpeed || 0).toFixed(0);
    instructionProgressBar.width = `${Math.min(100, Math.max(0, navigationState.progressAlongStep * 100))}%`;
  }
  ...
  function startNavigation() {
    ...
    navigationStatusPanel.classList.add("show");
  ...
  function stopNavigation() {
    ...
    clearRouteFromMap();
    resetNavigationState();
    navigationStatusPanel.classList.remove("show");
  ...
  async function handlePositionUpdate(position) {
    ...
    const headingDifference = heading == null ? 0 : Math.abs(heading - stepBearing);
    ...
    const routeTotalLen = turf.length(routeLine, { units: "meters" });
    const snappedDist = snapped.properties.location || 0;
    const progressRatio = routeTotalLen ? Math.min(1, Math.max(0, snappedDist / routeTotalLen)) : 0;
    const timeElapsed = tripDurationSeconds * progressRatio;
    const remainingTimeSeconds = Math.max(0, tripDurationSeconds - timeElapsed);
  ...
  function openSettings() {
    ...
    if (isMobile) { menuOverlay.classList.add("open"); menuOverlay.style.display = "block"; }
  }
  function closeSettings() {
    settingsMenu.classList.remove("open");
    if (isMobile) { menuOverlay.classList.remove("open"); menuOverlay.style.display = "none"; }
  }
  ...
  map.on("click", async (e) => {
    ...
    try {
      ...
      if (data && data.display_name && data.lat && data.lon) {
        closePanel();
        processPlaceResult({ display_name: data.display_name, lat: data.lat, lon: data.lon });
      }
    } catch (error) {
  ...
  // --- MOBILE BOTTOM SHEET DRAG (Google Maps-style) ---
  if (isMobile) {
    let startY = 0;
    let startBottom = 0;
    let dragging = false;
    ...
    const onTouchStart = (evt) => {
      ...
      const bottomVal = styles.bottom;
      startBottom = parseFloat(bottomVal);
    ...
    const onTouchMove = (evt) => {
      ...
      // invert because panel is positioned from bottom
      let newBottom = startBottom - deltaY;
      // clamp: between fully hidden (-100% + peek) and fully open (0)
      const minBottom = -(vh - parseFloat(getComputedStyle(sidePanel).height)) + parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--panel-mobile-peek"));
      newBottom = Math.min(0, Math.max(minBottom, newBottom));
    ...
    const settlePositions = () => {
      ...
      const openedThreshold = window.innerHeight * 0.45;
      if (window.innerHeight - rect.top > openedThreshold) {
        sidePanel.classList.add("open");
    ...
    document.getElementById("panel-grabber").addEventListener("touchstart", onTouchStart, { passive: true });
    document.getElementById("panel-grabber").addEventListener("touchmove", onTouchMove, { passive: true });
    document.getElementById("panel-grabber").addEventListener("touchend", onTouchEnd);
  }

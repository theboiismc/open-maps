...
let currentPlace = null; // Store data of the currently selected place
let routeLine = null;
...
function showPanel(viewId) {
    ...
    // Open the main panel container if it's not already
...
/**
 * Main function to process a selected place, fetch all data, and update the UI.
 * @param {object} place - The place object from Nominatim.
 */
function processPlaceResult(place) {
    currentPlace = place; // Save the selected place data
...
if (window.innerWidth <= 768) {
    ...
    grabber.addEventListener('touchend', () => {
        ...
        const threshold = -200; // Snap point

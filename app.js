...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    document.getElementById('info-save-btn').addEventListener('click', () => {
        if (currentUser) alert("Feature 'Save Place' not yet implemented!");
    ...
    shareRouteBtn.addEventListener('click', async () => {
        ...
        if (navigator.share) {
            try { await navigator.share({ title:'TheBoiisMC Maps Route', text:`Check out this route from ${fromName} to ${toName}!`, url: url.toString() }); }
            catch (error) { console.error('Error sharing:', error); }
        } else {
            navigator.clipboard.writeText(url.toString()).then(() => { alert("Route link copied to clipboard!"); })
            .catch(err => { console.error('Could not copy link: ', err); alert("Could not copy link. Please manually copy the URL."); });
        }
    });
    ...
    async function handlePositionUpdate(position) {
        ...
        if (heading!=null) map.easeTo({ center:snapped.geometry.coordinates, bearing:heading, zoom:18, duration:500 });
        else map.easeTo({ center:snapped.geometry.coordinates, zoom:18, duration:500 });
        ...
        if (snapped.properties.dist>50) { navigationState.isRerouting=true; speech.speak("Off route. Recalculating.",true); await getRoute(); return; }
        if (heading!=null && headingDifference>90 && headingDifference<270 && navigationState.userSpeed>5 && !navigationState.isWrongWay) { navigationState.isWrongWay=true; speech.speak("Wrong way. Recalculating.",true); await getRoute(); return; }
        ...
        if(distanceMiles>0.9 && distanceMiles<1.1 && navigationState.lastAnnouncedDistance>1.1){ speech.speak(`In 1 mile, ${currentStep.maneuver.instruction}`); navigationState.lastAnnouncedDistance=1; }
        else if(distanceMiles>0.24 && distanceMiles<0.26 && navigationState.lastAnnouncedDistance>0.26){ speech.speak(`In a quarter mile, ${currentStep.maneuver.instruction}`); navigationState.lastAnnouncedDistance=0.25; }

        if (navigationState.distanceToNextManeuver<50) {
            navigationState.currentStepIndex++;
            if (navigationState.currentStepIndex>=steps.length) { speech.speak("You have arrived at your destination.",true); stopNavigation(); return; }
    ...
    settingsBtns.forEach(btn=>{ btn.addEventListener('click', e=>{ e.stopPropagation(); settingsMenu.classList.add('open'); if(isMobile) menuOverlay.classList.add('open'); }); });
    closeSettingsBtn.addEventListener('click',()=>{ settingsMenu.classList.remove('open'); if(isMobile) menuOverlay.classList.remove('open'); });
    menuOverlay.addEventListener('click',()=>{ settingsMenu.classList.remove('open'); if(isMobile) menuOverlay.classList.remove('open'); });
    ...
    styleRadioButtons.forEach(radio=>{ radio.addEventListener('change', ()=>{ const newStyle=radio.value; map.setStyle(STYLES[newStyle]); if(isMobile) setTimeout(()=>settingsMenu.classList.remove('open'),200); }); });
    trafficToggle.addEventListener('change', ()=>{ if(trafficToggle.checked) addTrafficLayer(); else removeTrafficLayer(); if(isMobile) setTimeout(()=>settingsMenu.classList.remove('open'),200); });

    map.on('styledata', ()=>{ if(navigationState.isActive && currentRouteData){ addRouteToMap({ type:'Feature', geometry:currentRouteData.routes[0].geometry }); updateHighlightedSegment(currentRouteData.routes[0].legs[0].steps[navigationState.currentStepIndex]); } if(trafficToggle.checked) addTrafficLayer(); });
    ...
    map.on('click', async e=>{
        ...
        try {
            ...
            if(data) processPlaceResult(data,true);

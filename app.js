...
window.addEventListener('DOMContentLoaded', () => {
  ...
  getRouteBtn.addEventListener('click', async () => {
    ...
    const url = `https://router.project-osrm.org/route/v1/driving/${originLon},${originLat};${destinationLon},${destinationLat}?overview=full&geometries=geojson`;
    ...
    try {
      ...
      if (data.routes && data.routes.length > 0) {
        ...
        // Show distance and duration in route info
        const distKm = (route.distance / 1000).toFixed(2);
        const durationMin = Math.round(route.duration / 60);
        routeInfoDiv.textContent = `Distance: ${distKm} km — ETA: ${durationMin} min`;
      } else {
